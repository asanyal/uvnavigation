...
public class Node extends Entity implements Comparable<Node>
{
	 ...
	 * @deprecated As of 0.40, replaced by Node(entityData, latitude,
	 *             longitude).
	 */
	public Node(long id, int version, Date timestamp, OsmUser user,
			long changesetId, double latitude, double longitude)
	{
		// Chain to the more-specific constructor
		this(id, version, new SimpleTimestampContainer(timestamp), user,
				changesetId, latitude, longitude);
	}
	 ...
	 *             longitude).
	 */
	public Node(long id, int version, TimestampContainer timestampContainer,
			OsmUser user, long changesetId, double latitude, double longitude)
	{
		super(id, version, timestampContainer, user, changesetId);	//sets the entity data using the params passed
			...
			double longitude)
	{
		// Chain to the more-specific constructor
		this(id, version, new SimpleTimestampContainer(timestamp), user,
				changesetId, tags, latitude, longitude);
	}
	...
	public Node(StoreReader sr, StoreClassRegister scr)
	{
		...
		this.latitude = FixedPrecisionCoordinateConvertor.convertToDouble(sr
				.readInteger());
		this.longitude = FixedPrecisionCoordinateConvertor.convertToDouble(sr
				.readInteger());
	}
	...
	public void store(StoreWriter sw, StoreClassRegister scr)
	{
		...
		sw.writeInteger(FixedPrecisionCoordinateConvertor
				.convertToFixed(latitude));
		sw.writeInteger(FixedPrecisionCoordinateConvertor
				.convertToFixed(longitude));
	}
	...
	public int compareTo(Node comparisonNode)
	{
		...
		if (this.getTimestamp() == null
				&& comparisonNode.getTimestamp() != null)
		{
			return -1;
		}
		if (this.getTimestamp() != null
				...
				&& comparisonNode.getTimestamp() != null)
		{
			...
			result = this.getTimestamp().compareTo(
					comparisonNode.getTimestamp());
    ...
    public void setUvValue(double uvValue)
    {
