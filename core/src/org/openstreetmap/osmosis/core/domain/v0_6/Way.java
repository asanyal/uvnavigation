...
public class Way extends Entity implements Comparable<Way>
{
     ...
     * @deprecated As of 0.40, replaced by Way(entityData).
     */
    public Way(long id, int version, TimestampContainer timestampContainer,
            OsmUser user, long changesetId)
    {
    ...
    public int compareTo(Way comparisonWay)
    {
        ...
        if (this.getTimestamp() != null && comparisonWay.getTimestamp() != null)
        {
            ...
            result = this.getTimestamp()
                    .compareTo(comparisonWay.getTimestamp());
    ...
    /**
     * Is this way closed? (A way is closed if the first node id equals the last
     * node id.)
     * 
    ...
    {
        return wayNodes.get(0).getNodeId() == wayNodes.get(wayNodes.size() - 1)
                .getNodeId();
    }
