...
//                buffer.run();
    ...
    public void parseOsmChange(final ChangeSink sink) {
        ...
        try {
            ...
            if (e.getMessage().equals("null")) {
                // this seems to be an api0.5-file.
                org.openstreetmap.osmosis.core.xml.v0_5.XmlChangeReader oldTask = new org.openstreetmap.osmosis.core.xml.v0_5.XmlChangeReader(this.myFileName, true, compr);
                MigrateChangeV05ToV06 migrate = new MigrateChangeV05ToV06();
                migrate.setChangeSink(sink);
                oldTask.setChangeSink(migrate);
                oldTask.run();
            } else {
                throw e;
            }
