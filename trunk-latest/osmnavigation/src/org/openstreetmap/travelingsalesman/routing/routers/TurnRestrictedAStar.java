...
/**
 * Adaption of the {@link MultiTargetDijkstraRouter} to operate on RoutingSteps
 * instead of nodes.<br/>
 ...
 * With the {@link AStarComparator} this is actually A* and no longer Dijkstra.<br/>
 * 
 * We assume:
 * <ul>
...
public class TurnRestrictedAStar implements IRouter
{
    ...
    /**
     * Compare 2 routing-steps for sorting. Compare by: estimated cost from the
     * step + actual cost of the step + actual cost to the step
     */
    private final class AStarComparator implements Comparator<RoutingStep>
    {
         ...
         *            The start-node where our routing ends.
         * @param aBestDistancesFrom
         *            getStepKey(RoutingStep) -> summed cost to get to its
         *            endNode
         */
        ...
        /**
         * Estimate the cost before taking this step for comparison with the
         * actual cost.
         * 
         * @param aStep
         *            the step to calculate for
        ...
        public double getSummedHeuristic(final RoutingStep aStep)
        {
            ...
            {
                double dist = LatLon.distanceInMeters(aStep.getEndNode(),
                        targetNode);
        ...
        public double getHeuristic(final RoutingStep aStep)
        {
            ...
            double dist = LatLon.distanceInMeters(aStep.getStartNode(),
                    myStartingPoint);
            return dist;
        }
        ...
        /**
         * @param aStep
         *            the sum of cost, summed cost to get to aStep and estimated
         *            cost to get to the start-point
        ...
        public double getComparedValue(final RoutingStep aStep)
        {
            ...
            if (myBestStepsFrom.get(aStep) != null)
                LOG.log(Level.INFO, myBestStepsFrom.get(aStep).toString());
            ...
            // remainingEstimated = remainingEstimated * before /
            // beforeEstimated;
            // }
        ...
        public int compare(final RoutingStep stepA, final RoutingStep stepB)
        {
            ...
            return stepA.hashCode() - stepB.hashCode(); // never return 0 for
                                                        // different steps
        }
    }
     ...
     */
    private static final Node DUMMYTARGETNODE = new Node(
            (long) Integer.MIN_VALUE + 1, 0, new Date(), OsmUser.NONE, 0, 0, 0);
    ...
    private static final Way DUMMYWAY = new Way((long) Integer.MIN_VALUE + 1,
            0, new Date(), new OsmUser(1, "dummy"), 0);
    ...
    private static final Logger LOG = Logger
            .getLogger(TurnRestrictedAStar.class.getName());

    /**
     * used by {@link #progressMade(Node, Node, Node)} to see if an update for
     * our listeners is waranted. (we cannot inform them on every loop-step
    ...
    /**
     * Get the shortest of the distances from any of the target-nodes to the
     * start.
     * 
     * @param aTargetNodes
     *            the targets (any)
     ...
     *            the stat to reach
     * @return the distance
     * @see LatLon#distance(Node,
     *      org.openstreetmap.osm.data.coordinates.Coordinate)
     */
    private double getShortestDistance(final Collection<Node> aTargetNodes,
            final Node start)
    {
     ...
     *            where we are
     * @param aTargetNodes
     *            where we are going (any of them)
     * @param start
     *            where we are started
     */
    protected void progressMade(final Node here,
            final Collection<Node> aTargetNodes, final Node start)
    {
     ...
     * @return the metric
     */
    private double calculateDistance(final IDataSet aMap,
    ...
    /**
     * @param aTargetNodes
     *            the {@link Node}s, any one of wich we want to reach
     * @param aStartingPoint
     *            the {@link Node} we are now at
     * @param aSelector
     *            (may be null) optional selector to determine unallowed roads.
     * @param aMap
     *            the map to route on
     ...
     */
    public Route route(final IDataSet aMap,
            final Collection<Node> aTargetNodes, final Node aStartingPoint,
            final IVehicle aSelector)
    {
        ...
        LOG.log(Level.INFO,
                "===================================================");
        LOG.log(Level.INFO,
                "TurnRestrictedMultiTargetDijkstraRouter starting...");

        final Map<String, Double> bestDistancesFrom = new HashMap<String, Double>(); // Step-Key->best
                                                                                     // Metric
                                                                                     // of
                                                                                     // a
                                                                                     // route
                                                                                     ...
                                                                                     // so
                                                                                     // far
        Map<String, RoutingStep> bestStepsFrom = new HashMap<String, RoutingStep>(); // last
                                                                                     // routing-step
                                                                                     // taken
                                                                                     // to
                                                                                     // reach
        ...
        AStarComparator comparator = new AStarComparator(aTargetNodes,
                aStartingPoint, bestDistancesFrom, bestStepsFrom);
        ...
        // ------------------------------------------------------------------------
        // add all target-nodes reachable from the single dummy target-node
        // (multi-target -routing)
        // to bestDistances using dummy RoutingSteps
        for (Node target : aTargetNodes)
        {
            Way dummyway = DUMMYWAY;
            Collection<RoutingStep> nextSteps = getNextNodes(aMap,
                    aStartingPoint, new RoutingStep(aMap, target,
                            DUMMYTARGETNODE, dummyway), stepKeysVisited,
                    aSelector);
            for (RoutingStep nextStep : nextSteps)
            {
                ...
                if (stepKeysVisited.contains(stepKey))
                {
                    throw new IllegalStateException(
                            "getNextNodes returned a step we already evaluated");
                }
                bestStepsFrom.put(stepKey, nextStep);
                bestDistancesFrom.put(stepKey,
                        (double) (nextStep.distanceInMeters()));
                // bestDistancesFrom.put(stepKey, calculateDistance(aMap, null,
                // nextStep));
                stepsToVisit.put(comparator.getComparedValue(nextStep),
                        nextStep);
            }
        }

        // ------------------------------------------------------------------------
        // try to find better ways to each RoutingStep in bestDistances from the
        // target
        while (!stepsToVisit.isEmpty())
        {
            final Entry<Double, RoutingStep> currentEntry = stepsToVisit
                    .pollFirstEntry();
            ...
            LOG.log(Level.INFO,
                    "=================================================== We are at: "
                            ...
                            + currentEntry.getKey() + " )");
            LOG.info("Forbidding further evaluation of "
                    + getStepKey(currentStep) + " #stepsToVisit="
                    + stepsToVisit.size() + " =" + stepKeysVisited.hashCode());
            if (currentStepNode.getId() == aStartingPoint.getId())
            {
                // return with shortest path
                LOG.log(Level.INFO,
                        "found a shortest path, reconstructing path...");
                return reconstructShortestPath(aMap, aTargetNodes, currentStep,
                        bestStepsFrom, bestDistancesFrom);
            }
            Collection<RoutingStep> nextSteps = getNextNodes(aMap,
                    aStartingPoint, currentStep, stepKeysVisited, aSelector);
            progressMade(currentStepNode, aTargetNodes, aStartingPoint);

            double bestDistanceFromCurrentNode = bestDistancesFrom
                    .get(getStepKey(currentStep));
            ...
            for (RoutingStep nextStep : nextSteps)
            {
                ...
                LOG.info("Evaluating step from node " + nextNode.getId()
                        + " via way " + nextStep.getWay().getId() + " to node "
                        + nextStep.getEndNode().getId());

                assert (nextStep.getEndNode().getId() == currentStepNode
                        .getId()) : "Dijkstra: getNextNodes returned a step that does not end where it should!!!\n"
                        + "should end at: "
                        + currentStepNode.getId()
                        + "\n"
                        + "does   end at: "
                        + nextStep.getStartNode().getId()
                        + "\n"
                        + "does start at: "
                        + nextStep.getEndNode().getId() + "\n";
                assert (nextNode.getId() != currentStepNode.getId()) : "Dijkstra: getNextNodes returned a step that does START where it should END!!!\n"
                        + "should end at: "
                        + currentStepNode.getId()
                        + "\n"
                        + "does   end at: "
                ...
                double calculatedDistance = calculateDistance(aMap,
                        currentStep, nextStep);
                boolean alreadyVisited = bestDistancesFrom
                        .containsKey(getStepKey(nextStep));
                if (!alreadyVisited
                        || bestDistancesFrom.get(getStepKey(nextStep)) > bestDistanceFromCurrentNode
                                + calculatedDistance)
                {
                    ...
                    bestDistancesFrom.put(getStepKey(nextStep),
                            bestDistanceFromCurrentNode + calculatedDistance);
                    bestStepsFrom.put(getStepKey(nextStep), currentStep);

                    if (alreadyVisited)
                    {
                        // this happens very seldomly andd taked 8% of the time
                        // in a city
                        LOG.log(Level.FINE, "resorting stepsToVisit, length="
                                + stepsToVisit.size());
        ...
        LOG.log(Level.INFO,
                "TurnRestrictedMultiTargetDijkstraRouter found nothing");
     ...
     *            all elements
     * @param currentStep
     *            the presumed lowest cost element
            ...
            final RoutingStep currentStep)
    {
        ...
        // Iterator<Entry<Double, RoutingStep>> iterator =
        // stepsToVisit.entrySet().iterator();
        ...
        // LOG.warning("Metric has changed from " + entry.getKey() + " to " +
        // metric);
        // }
        ...
        // LOG.warning("Implementation error. next step is not optimal (" +
        // currentMetric + " vs " + bestMetric + ")");
     ...
     * @param aTargetNodes
     *            where we are going (any of these points is fine)
     * @param aStartingStep
     *            where we started
     * @param bestStepsFrom
     *            the best last-step indexed by the stepKey of the step we come
     *            from
     * @param aBestDistances
     *            - for debugging only
     * @return the route
     */
    private Route reconstructShortestPath(final IDataSet aMap,
            final Collection<Node> aTargetNodes,
            final RoutingStep aStartingStep,
            final Map<String, RoutingStep> bestStepsFrom,
            final Map<String, Double> aBestDistances)
    {
        ...
        while (!targetNodeIDs.contains(currentStep.getEndNode().getId()))
        {
            ...
            if (bestStep.getStartNode().getId() != currentStep.getEndNode()
                    .getId())
            {
                throw new IllegalStateException(
                        "I TurnRestrictedMultiPathDijkstra's bestStep we have a step that does not start where it should!!\n"
                                + "should start at: "
                                + currentStep.getEndNode().getId()
                                + "\n"
                                + "does   start at: "
            ...
            if (lastStep != null
                    ...
                    && lastStep.getStartNode().getId() != currentStep
                            .getEndNode().getId())
            {
                ...
                assert !(lastStep.getStartNode().getId() == bestStep
                        ...
                        .getStartNode().getId()) : "Found a loop in MultiPathDijkstra!!\n"
                        + "Node: "
                        ...
                        + " (walking best-steps from target to start)\n";
                // //DEBUG---------------
                // if (lastStep.getStartNode().getId() ==
                // bestStep.getEndNode().getId()
                // && lastStep.getEndNode().getId() ==
                // bestStep.getStartNode().getId()) {
                // double distA =
                // Coordinate.distance(lastStep.getStartNode().getLatitude(),
                // lastStep.getStartNode().getLongitude(),
                // aTargetNode.getLatitude(), aTargetNode.getLongitude());
                // double distB =
                // Coordinate.distance(bestStep.getStartNode().getLatitude(),
                // bestStep.getStartNode().getLongitude(),
                // aTargetNode.getLatitude(), aTargetNode.getLongitude());
                // throw new
                // IllegalStateException("Found a loop in Dijkstra!!\n"
                // + "Node: " + lastStep.getStartNode().getId() +
                // " dist-to-target=" + distA + " road-dist-to-target="
                // + aBestDistances.get(lastStep.getStartNode().getId()) + "\n"
                // + "Node: " + bestStep.getStartNode().getId() +
                // " dist-to-target=" + distB + " road-dist-to-target="
                // + aBestDistances.get(bestStep.getStartNode().getId()) + "\n"
                // + "current Step: " + bestStep.getStartNode().getId() + " -> "
                // + bestStep.getEndNode().getId() +
                // " (walking best-steps from target to start)\n"
                // + "last    Step: " + lastStep.getStartNode().getId() + " -> "
                // + lastStep.getEndNode().getId() +
                // " (walking best-steps from target to start)\n"
    ...
    /**
     * @param aStartingPointToReach
     *            the {@link Node} we want to reach
     * @param aCurrentStep
     *            where we are (startNode) and have gone to (toNode)
     * @param forbiddenStepKeys
     *            the {@link RoutingStep}s we already stepped on
     * @param aMap
     *            the map to route in
     * @param aSelector
    ...
    protected Collection<RoutingStep> getNextNodes(final IDataSet aMap,
            final Node aStartingPointToReach, final RoutingStep aCurrentStep,
            final Set<String> forbiddenStepKeys, final IVehicle aSelector)
    {
        ...
        try
        {
            ...
            while (waysForNode != null && waysForNode.hasNext())
            {
                ...
                Collection<Relation> turnRestrictionsFrom = getTurnRestrictionsFrom(
                        aMap, way, aCurrentStep.getStartNode());
                if (!isAllowedTurn(turnRestrictionsFrom, aCurrentStep, way))
                {
                    LOG.log(Level.FINE, "turn-restriction forbids to go "
                            + "from way " + way.getId() + " via node "
                            + aCurrentStep.getStartNode().getId() + " to way "
                            + aCurrentStep.getWay().getId() + " to node "
                            + aCurrentStep.getEndNode().getId());
                    continue;
                }
                ...
                try
                {
                    ...
                    int index = getNodeIndex(aCurrentStep.getStartNode(),
                            wayNodeList);
                            ...
                            && !aSelector.isOneway(aMap, way))
                    {
                        ...
                        if (node == null)
                        {
                            LOG.log(Level.SEVERE,
                                    "could not load node with ID=" + nodeID);
                            continue;
                        }
                        ...
                        RoutingStep step = new RoutingStep(aMap, node,
                                aCurrentStep.getStartNode(), way);
                        if ((!forbiddenStepKeys.contains(getStepKey(step)))
                                && aSelector.isAllowed(aMap, node))
                        {
                            LOG.info("Scheduling step ahead from node "
                                    + node.getId() + " via way " + way.getId()
                                    + " to "
                                    + aCurrentStep.getStartNode().getId());
                            retval.add(step); // we are going FROM node TO
                                              // aCurrentNode!
                        }
                    } else // turn around a roundabout
                    if (wayNodeList.size() == index + 1
                            && wayNodeList.size() > 2
                            && wayNodeList.get(0).getNodeId() == wayNodeList
                                    .get(wayNodeList.size() - 1).getNodeId()
                            && !aSelector.isOneway(aMap, way))
                    {
                        ...
                        if ((!forbiddenStepKeys.contains(getStepKey(step)))
                                && aSelector.isAllowed(aMap, node))
                            LOG.info("Scheduling step around from node "
                                    + node.getId() + " via way " + way.getId()
                                    + " to "
                                    + aCurrentStep.getStartNode().getId());
                        retval.add(step); // we are going FROM node TO
                    ...
                    index = getLastNodeIndex(aCurrentStep.getStartNode(),
                            wayNodeList);
                    if (index > 0 && !aSelector.isReverseOneway(aMap, way))
                    {
                        ...
                        if ((!forbiddenStepKeys.contains(getStepKey(step)))
                                && aSelector.isAllowed(aMap, node))
                        {
                            LOG.info("Scheduling step back from node "
                                    + node.getId() + " via way " + way.getId()
                                    + " to "
                                    + aCurrentStep.getStartNode().getId()
                                    + " (stepkey=" + getStepKey(step)
                                    + ") forbiddenStepKeys="
                                    + forbiddenStepKeys.hashCode());
                            retval.add(step); // we are going FROM node TO
                                              // aCurrentNode!
                        }
                    }
                } catch (Exception e)
                {
                    LOG.log(Level.SEVERE, "Exception while doing way #" + way
                            + " way in getNextNodes(NodeID="
                            + aCurrentStep.getStartNode().getId()
                            + ") in MultiTargetDijkstra! "
                            + "Ignoring this way.", e);
                }
            }
        } catch (Exception e)
        {
            LOG.log(Level.SEVERE,
                    "Exception while doing getNextNodes(NodeID="
                            + aCurrentStep.getStartNode().getId()
                            + ") in TurnRestrictedMultiTargetDijkstra! Considering this node a dead end.",
                    e);
        }
     ...
     * 
     * @param aTurnRestrictionsFrom
     *            all known turn-restrictions involving aFromWay
     * @param aToStep
     *            where we go to
     * @param aFromWay
     *            where we come from
    ...
    private boolean isAllowedTurn(
            ...
            final RoutingStep aToStep, final Way aFromWay)
    {
        try
        {
            ...
            {
                String type = WayHelper.getTag(relation.getTags(),
                        "restriction");
                if (type == null)
                {
                    LOG.info("incomplete turn-restriction " + relation.getId()
                            + " as no restriction-attribute");
                    continue;
                }
                ...
                {
                    LOG.info("illegal turn-restriction " + relation.getId()
                            + " has type `" + type
                            + "` that does not start with only or no");
                    continue;
                }
                    ...
                    for (RelationMember member : members)
                    {
                        if (member.getMemberRole() != null
                                ...
                                && member.getMemberType()
                                        .equals(EntityType.Way))
                        {
                            if (aToStep.getWay().getId() == member
                                    .getMemberId())
                            {
        ...
        {
            LOG.log(Level.SEVERE, "Exception while checking turn-restrictions",
                    e);
        }
     ...
     * @param aFromWay
     *            the way we are comming from.
     * @param aViaNode
     *            the node of the intersection
     * @return the turn-restrictions
     */
    private Collection<Relation> getTurnRestrictionsFrom(final IDataSet aMap,
            final Way aFromWay, final Node aViaNode)
    {
        ...
        for (Long relID : referencedRelationIDs)
        {
            ...
            for (RelationMember relationMember : members)
            {
                if (relationMember.getMemberRole() != null
                        && relationMember.getMemberRole().equalsIgnoreCase(
                                "via")
                        && relationMember.getMemberType().equals(
                                EntityType.Node))
                {
                    hasViaNodes = true;
            ...
            {
                LOG.fine("NOT ignoring turn-restriction because of via-node "
                        + aViaNode.getId() + " is matching");
            }
     ...
     */
    private int getLastNodeIndex(final Node aStartNode,
            final List<WayNode> wayNodeList)
    {
        ...
        if (lastIndex == -1
                && wayNodeList.size() > 2
                && wayNodeList.get(0).getNodeId() == wayNodeList.get(
                        wayNodeList.size() - 1).getNodeId()
