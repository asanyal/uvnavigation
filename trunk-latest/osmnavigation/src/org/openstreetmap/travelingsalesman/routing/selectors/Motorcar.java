...
public final class Motorcar implements IVehicle
{
    ...
    private static final Logger LOG = Logger
            .getLogger(Motorcar.class.getName());
    ...
    /**
     * Setting the user can use to specify a minimum weight for the street to
     * carry.
     */
    ...
    public boolean isAllowed(final IDataSet aMap, final Node aNode)
    {
        ...
        if (carAccess != null
                && (carAccess.equalsIgnoreCase("no") || carAccess
        ...
        if (width != null)
        {
            double configured = Settings.getInstance().getDouble(SETTING_WIDTH,
                    0);
            if (configured > 0)
            {
                try
                ...
                {
                    LOG.log(Level.WARNING, "Node " + aNode.getId()
                            + " with illegal " + UsedTags.TAG_WIDTH
                            + "-tag found");
                }
            }
        }
        ...
        if (width != null)
        {
            double configured = Settings.getInstance().getDouble(
                    SETTING_WEIGHT, 0);
            if (configured > 0)
            {
                try
                ...
                {
                    LOG.log(Level.WARNING, "Node " + aNode.getId()
                            + " with illegal " + UsedTags.TAG_WEIGHT
                            + "-tag found");
                }
            }
        }
        ...
        if (height != null)
        {
            ...
            if (configured > 0)
            {
                ...
                {
                    LOG.log(Level.WARNING, "Node " + aNode.getId()
                            + " with illegal " + UsedTags.TAG_HEIGHT
                            + "-tag found");
                }
            }
        }
     ...
     * @param aMap
     *            the map we operate on.
     * @return true if this is a one-way-street in the opposite direction
     */
    ...
    /**
     * See http://wiki.openstreetmap.org/index.php/Map_Features#Highway for
     * details on the tags we test.
     * 
     * @return true if this node can be traveled by car.
     * @param aWay
     *            the way to test
    ...
    public boolean isAllowed(final IDataSet aMap, final Way aWay)
    {
        ...
        // explicitely allowed
        if (carAccess != null
                && (carAccess.equalsIgnoreCase("yes") || carAccess
        ...
        if (width != null)
        {
            ...
            if (configured > 0)
            {
                ...
                {
                    LOG.log(Level.WARNING, "Way " + aWay.getId()
                            + " with illegal " + UsedTags.TAG_WIDTH
                            + "-tag found");
                }
            }
    ...
    static
    {
        ...
        // these are to be used on nodes but by mistake may have been added to a
        // way
    ...
    public ConfigurationSection getSettings()
    {
        ...
        settings.addSetting(new ConfigurationSetting(SETTING_WEIGHT,
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.weight.title"),
                TYPES.DOUBLE, OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.weight.category"),
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.weight.desc")));
        
        settings.addSetting(new ConfigurationSetting(SETTING_WIDTH,
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.width.title"),
                TYPES.DOUBLE, OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.width.category"),
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.width.desc")));
        
        settings.addSetting(new ConfigurationSetting(SETTING_HEIGHT,
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.height.title"),
                TYPES.DOUBLE, OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.height.category"),
                OsmNavigationConfigSection.RESOURCE
                        .getString("vehicle.motorcar.height.desc")));
