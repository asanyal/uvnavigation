...
public class Route
{
    ...
    /**
     * A Routing-Step is a part of a route that is between 2 nodes on the same
     * way.
     * 
     * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
     */
    public static class RoutingStep
    {
        ...
        /**
         * What way {@link #myStartNode} and {@link #myEndNode} are.
         */
        private Way myWay;
         ...
         * UV Parameters
         */
        private double uvLeft;
        private double uvRight;

        /**
         * 
         * @return the UV magnitude of the left path
         */
        ...
        /**
         * 
         * @return the UV magnitude of the right path
         */
         ...
         *            where we arrive
         * @param aWay
         *            what way both nodes are on
         */
        public RoutingStep(final IDataSet aMap, final Node aStartNode,
                final Node aEndNode, final Way aWay)
        {
            if (aMap == null)
            {
        ...
        public void setStartNode(final Node aStartNode)
        {
            if (myEndNode.getId() == aStartNode.getId())
                throw new IllegalArgumentException(
                        "start-node must not be the end-node!");
            myStartNode = aStartNode;
            myNodesOnWay = null;
        }
        ...
        /**
         * @return list of segments comprising of this Routing Step author@
         *         Atindriyo Sanyal
         */
        public List<ISegment> getSegments()
        {
            List<ISegment> mySegment = new LinkedList<ISegment>();
            ...
            {
                if (node.getNodeId() == getEndNode().getId())
                {
                    endIndex = index;
                    if (startIndex >= 0)
                    {
            ...
            /*
             * Nodes in the Map are ordered in increasing order of Node ID E.g
             * the lowers node id is at the top left highest node id is at the
             * bottom right
             */
            // order of Node ID are increasing eg : 1 2 3 4 5...
            if (endIndex > startIndex)
            {
                for (int i = startIndex; i < endIndex; i++)
                {
                    Segment s = new Segment(wayNodes.get(i).getNodeId(), 0,
                            new Date(), null, 0, wayNodes.get(i),
                            wayNodes.get(i + 1));
                    mySegment.add(s);
                }
            } else
            {
                // order of Node ID are decreasing eg : 5 4 3 2 1.....
                for (int i = startIndex; i > endIndex; i--)
                {
                    Segment s = new Segment(wayNodes.get(i).getNodeId(), 0,
                            new Date(), null, 0, wayNodes.get(i),
        ...
        /**
         * Get all nodes on the way in the order they are passed.
         * 
        ...
        public List<WayNode> getNodes()
        {
            ...
            if (wayNodes.get(0).getNodeId() != wayNodes
     ...
     * @param aMap
     *            The map that is routed on.
     * @param aRoutingStepList
     *            The ordered list of {@link RoutingStep}s from the start to our
     *            destination.
     * @param aStartNode
     *            The node where this route starts.
     */
    public Route(final IDataSet aMap, final List<RoutingStep> aRoutingStepList,
            final Node aStartNode)
    {
        super();
        if (aMap == null)
            throw new IllegalArgumentException("null map given");
     ...
     * 
     * @param aRoutes
     *            the routes to combine. The start of one route needs to be the
     *            target of the previous route.
    ...
    public static Route combine(final List<Route> aRoutes)
    {
        ...
        for (Route route : aRoutes)
        {
            if (route == null)
                throw new IllegalArgumentException(
                        "one of the given routes is null!");
