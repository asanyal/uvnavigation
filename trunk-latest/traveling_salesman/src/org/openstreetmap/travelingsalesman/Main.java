...
import org.openstreetmap.travelingsalesman.routing.metrics.LeastUVMetric;
...
import org.openstreetmap.travelingsalesman.routing.routers.UVDijkstraRouter;
...
public final class Main {
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            // ------------------------------- load default-map from server or
            // local file
            String defaultDir = Settings.getInstance().get(
                    "map.dir",
                    Settings.getInstance().get(
                            "tiledMapCache.dir",
                            System.getProperty("user.home") + File.separator
                                    + ".openstreetmap" + File.separator + "map"
                                    + File.separator));
            ...
            if (!dir.exists()) {
                ...
                // IDataSet osmData = new FileTileDataSet(); // new
                // LODDataSet();
                // IDataSet osmData = new OsmBinDataSet(new
                // File("d:\\osmbin.data")); //DBDataSet(); // new LODDataSet();
                osmData = Settings.getInstance().getPlugin(IDataSet.class,
            ...
            if (args.length != 0) {
                ...
                while (args.length > argIndex) {
                    if (args[argIndex].equalsIgnoreCase("--loglevel")
                        ...
                        System.out.println("setting log-level for \"" + pkg
                                + "\" to \"" + level + "\"");
                    ...
                    if (args[argIndex].equalsIgnoreCase("--override")
                        ...
                        System.out.println("overriding setting for \"" + key
                                + "\" twith \"" + value + "\"");
                    ...
                    if (args[argIndex].equalsIgnoreCase("--set")
                        ...
                        String key = args[argIndex + 1];
                        String value = args[argIndex + 2];
                        System.out.println("setting setting for \"" + key
                                + "\" twith \"" + value + "\"");
                        Settings.getInstance().override(key, value);
                        argIndex += 3;
                        continue;
                    ...
                    if (args[argIndex].equalsIgnoreCase("route")) {
                        ...
                        System.err.println("unrecognized command or option: "
                                + args[argIndex]);
                        displayCmdlnUsage();
                        return;
                    }
     ...
     * @param argIndex
     *            the index of the "route"-command in the argument-list
     * @param aOsmData
     *            the map to use
     */
    private static void handleImport(final String[] aArgs, final int argIndex,
            final IDataSet aOsmData) {
        System.out.println("importing " + aArgs[argIndex + 1]);
        try {
            if (aArgs[argIndex + 1].toLowerCase().startsWith("http")
                    || aArgs[argIndex + 1].toLowerCase().startsWith("ftp")) {
                LoadMapFileActionListener.loadMapURL(null, new URL(
                        aArgs[argIndex + 1]), null, null, aOsmData);
            } else {
                LoadMapFileActionListener.loadMapFile(null, new File(
                        aArgs[argIndex + 1]), null, null, aOsmData);
            }
            aOsmData.shutdown();
        } catch (Exception e) {
     ...
     *            the map to use
     */
    private static void handleRouteCommand(final String[] aArgs,
            final int argIndex, final IDataSet aMap) {
        IDataSet map = aMap;
        // IRouter router = new DijkstraRouter();
        IRouter router = new UVDijkstraRouter();
        IVehicle vehicle = new Pedestrian();
        // IRoutingMetric metric = new StaticFastestRouteMetric();
        IRoutingMetric metric = new ShortestRouteMetric();
        ...
        while (parameterParsed) {
            ...
            if (aArgs[firstNodeArg].equals("-loadmap")) {
                FileLoader loader = new FileLoader(new File(
                        aArgs[firstNodeArg + 1]));
                map = loader.parseOsm();
                firstNodeArg += 2;
            ...
            if (aArgs[firstNodeArg].equals("-router")) {
                try {
                    router = (IRouter) Class.forName(aArgs[firstNodeArg + 1])
                            .newInstance();
                } catch (Exception e) {
                    System.err
                            .println("Cannot used given router-class. Using default router.");
                    e.printStackTrace();
                }
                firstNodeArg += 2;
                parameterParsed = true;
            ...
            if (aArgs[firstNodeArg].equals("-vehicle")) {
                try {
                    vehicle = (IVehicle) Class.forName(aArgs[firstNodeArg + 1])
                            .newInstance();
                } catch (Exception e) {
                    System.err
                            .println("Cannot used given vehicle-class. Using default router.");
                    e.printStackTrace();
                }
                firstNodeArg += 2;
                parameterParsed = true;
            ...
            if (aArgs[firstNodeArg].equals("-metric")) {
                try {
                    metric = (IRoutingMetric) Class.forName(
                            aArgs[firstNodeArg + 1]).newInstance();
                } catch (Exception e) {
                    System.err
                            .println("Cannot used given metric-class. Using default router.");
                    e.printStackTrace();
                }
                firstNodeArg += 2;
                parameterParsed = true;
        ...
        if (outFormat.equalsIgnoreCase(GPX)) {
            ...
            out.println("<gpx version=\"1.0\" creator=\"Traveling Salesman "
                    + MainFrame.RESOURCE.getString("Main.Version")
                    + "\" "
        ...
        for (int i = 0; i < nodes.length - 1; i++) {
            ...
            if (outFormat == GPX) {
                //print data to gpx file
                System.out.println("PRINTING TO GPX");
                for (RoutingStep step : routingSteps) {
                    Node endnode = step.getEndNode();
                    System.out.println(endnode.getId());
                    out.println("<trkpt lat=\"" + endnode.getLatitude()
                            + "\" lon=\"" + endnode.getLongitude() + "\" >");
                    out.println("<name>"
                            + NameHelper.getNameForNode(aMap, endnode)
                            + "</name>");
    ...
    /**
     * Whenever a node is given on the command-line, it can be given in various
     * formats. Determine the format and find the node.
     * 
     * @param aMap
     ...
     * @return the nodeID
     */
    private static long parseNodeCmdln(final IDataSet aMap,
            final String aNodeDesc) {
        if (aNodeDesc.startsWith("[") && aNodeDesc.endsWith("]")) {
            // node is given as [lat, lon]
            String[] latLon = aNodeDesc.substring(1, aNodeDesc.length() - 1)
                    .split(",");
            LatLon location = new LatLon(Double.parseDouble(latLon[0]),
                    Double.parseDouble(latLon[1]));
            Node nearestNode = aMap.getNearestNode(location, new Motorcar());
            if (nearestNode == null) {
                System.err.println("There is no node near " + aNodeDesc
                        + " in out map.");
                System.exit(-1);
            }
    ...
    private static void displayCmdlnUsage() {
        ...
        System.err
                .println("java -jar travelingSalesman.jar : start graphical navigator");
        System.err
                .println("java -jar travelingSalesman.jar [options] route [-router <classname>] [-metric <classname>] [-vehicle <classname>] "
        ...
        System.err
                .println("java -jar travelingSalesman.jar  [options] import <filename>");
        System.err
                .println("java -jar travelingSalesman.jar  [options] import <file-URL>");
        System.err.println("java -jar travelingSalesman.jar  [options] gui");
        System.err
                .println("options: (you may specify any number of them including none");
        System.err.println("--loglevel <package> <level>");
        System.err
                .println("   level may be: SEVERE, WARNING, INFO, FINE, FINER or FINEST");
        System.err.println("--set <key> <value> (permanent config-setting)");
        System.err
                .println("--override <key> <value>  (temporary config-setting)");
    }

    /**
     * Force the database to be opened and to replay all logs by requesting a
     * single node.
     * 
     * @param osmData
     *            the database
     */
    private static void forceDatabaseOpen(final IDataSet osmData,
        ...
        if (dialog) {
            ...
            JTextArea pleaseWaitLabel = new JTextArea(
    ...
    private static void configureLoggingPopup() {
        ...
        rootLogger.addHandler(new Handler() {
            ...
            public void publish(final LogRecord aRecord) {
                try {
                    if (aRecord.getLevel().intValue() >= Level.SEVERE
                            .intValue()) {
                        Runnable asyncPopup = new Runnable() {
                            public void run() {
                                ...
                                ErrorDialog.errorHappened(aRecord.getMessage(),
                                        message);
                        ...
                        NavigationManager.getExecutorService().execute(
                                asyncPopup);
                        // asyncPopup.run();
                    }
                } catch (Exception e) {
    ...
    private static void configureLoggingLevelAll() {
        ...
        Logger.getLogger("org.openstreetmap.travelingsalesman.routing.routers")
                .setLevel(Level.WARNING);
        Logger.getLogger(
                "org.openstreetmap.travelingsalesman.routing.describers")
                .setLevel(Level.FINER);
        // areas currently in development/debugging
        Logger.getLogger("org.openstreetmap.travelingsalesman.trafficblocks")
                .setLevel(Level.FINER);
        ...
        Logger.getLogger("org.openstreetmap.travelingsalesman.gps.GpsDProvider")
                .setLevel(Level.FINE);
        ...
        Logger.getLogger("org.openstreetmap.travelingsalesman.painting")
                .setLevel(Level.WARNING);
