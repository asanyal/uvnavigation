...
/**
 * The MapPanel exists to render a rectangular area of
 * the DataSet.
 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
 */
public class MapPanel extends SimpleMapPanel implements IRouteChangedListener, IExtendedGPSListener, ITracksChangeListener {
    /**
     * generated.
     */
    ...
    /**
     * The route that shall be marked in the map.
     * (May be null.)
     */
    ...
    /**
     * The next position the user is supposed
     * to act to mark.
     * (May be null.)
     */
    ...
    /**
     * Paint all the vector-data that we draw on top of the
     * actual map like the current location, the route,... .
     * @param g2d what to paint to
     */
    protected void paintAdditionalVectorData(final Graphics2D g2d) {
        if (getCurrentRoute() != null) {
            this.getMyPainter().setGraphics(g2d);
            List<RoutingStep> routeSegments = getCurrentRoute().getRoutingSteps();
            LOG.log(Level.FINEST, "paint() visiting " + routeSegments.size() + " route-segments");
        ...
        if (getCurrentPosition() != null) {
            this.getMyPainter().setGraphics(g2d);
//            if (autoRotationMode) {
//                this.getMyPainter().visitCurrentPosition(getCurrentPosition(), 0, getCurrentSpeed());
//            }   else {
                this.getMyPainter().visitCurrentPosition(getCurrentPosition(), getCurrentCourse(), getCurrentSpeed());
//            }
        }

        if (this.getNextManeuverPosition() != null) {
            this.getMyPainter().setGraphics(g2d);
            this.getMyPainter().visitNextManeuverPosition(this.getNextManeuverPosition());
        }
        ...
        Collection<TrafficMessage> allMessages = TrafficMessageStore.getInstance().getAllMessages(getDataSet());
    ...
    */
     ...
     * @param aSelectedNodePosition node postion
     */
    ...
    /**
     * @param aCurrentRoute the route we shall highlight in the map.
     */
    ...
    public void setCurrentPosition(final LatLon aCurrentPosition) {
        ...
        if (isAutoRotationMode() || isAutoCenterMode()) {
            zoomTo(getProjection().latlon2eastNorth(aCurrentPosition), getScale());
        } else {
    //       repaint only of it was or now is in the bounding-box displayed
            Bounds mapBounds = getMapBounds();
            if (oldPos != null && (mapBounds.contains(oldPos.lat(), oldPos.lon())
             || mapBounds.contains(aCurrentPosition.lat(), aCurrentPosition.lon()))) {
                repaint();
            }
        }
    }
    ...
    /**
     * {@inheritDoc}
     * @see org.openstreetmap.travelingsalesman.gps.IGPSProvider.IExtendedGPSListener#gpsDateTimeChanged(long, long)
     */
    ...
    /**
     * {@inheritDoc}
     * @see org.openstreetmap.travelingsalesman.gps.IGPSProvider.IExtendedGPSListener#gpsDopChanged(double, double, double)
     */
