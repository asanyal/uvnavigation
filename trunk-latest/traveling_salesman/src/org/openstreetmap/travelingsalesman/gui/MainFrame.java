...
//    private static final int READBUFFERSIZE = 255;
    ...
    /**
     * The default used for the zoom-level in
     * {@link #jumpTo(Place)}.
     */
    ...
    /**
     * The NavigationManager to simplify implementing
     * the calculation and updating of the route as
     * the driver moves along.
     */
    ...
    /**
     * The task-pane that make up the left
     * side of the UI.
     */
    private JTaskPane leftPanel = null;

    /**
     * The panel will all components used
     * for navigtion.
     */
    private JPanel navigationPanel = null;

    /**
     * The panel with the placefinder and
     * placesToGo.
     */
    //private JPanel navigationResultsPanel = null;

    /**
     * The Place-Finder -component allows
     * to search for targets to
     * navigate to.
     */
...
//    private IntersectionPanel intersectionPanel = null;
     ...
     */
    private IVehicle myVehicle = Settings.getInstance().getPluginProxy(IVehicle.class, Motorcar.class.getName());
    ...
    public MainFrame() {
        super();
        initialize();
        LOG.log(Level.INFO, "[JRE] Java version : " + System.getProperty("java.version"));
        LOG.log(Level.INFO, "[JRE] Java home : " + System.getProperty("java.home"));
    }
    ...
    private void initialize() {
        ...
        myGPSProvider = Settings.getInstance().getPluginProxy(IGPSProvider.class, DummyGPSProvider.class.getName());

        this.addWindowListener(new java.awt.event.WindowAdapter() {
            public void windowClosing(final WindowEvent winEvt) {
                Settings settings = Settings.getInstance();
                if (settings.getBoolean("traveling-salesman.restoreLastState", DEFAULTRESTORESTATEVALUE)) {
                    LOG.log(Level.FINE, "TravelingSalesman is closing. Saving state...");
                    settings.put("state.MainFrame.width",    String.valueOf(winEvt.getWindow().getWidth()));
                    settings.put("state.MainFrame.height",   String.valueOf(winEvt.getWindow().getHeight()));
                    settings.put("state.MainFrame.x",        String.valueOf(winEvt.getWindow().getX()));
                    settings.put("state.MainFrame.y",        String.valueOf(winEvt.getWindow().getY()));
//                    settings.put("state.MapPanel.east",     String.valueOf(MainFrame.this.getMapPanel().getCenter().east()));
//                    settings.put("state.MapPanel.north",    String.valueOf(MainFrame.this.getMapPanel().getCenter().north()));
                    final LatLon mapPanelCenter = getMapPanel().getProjection().eastNorth2latlon(getMapPanel().getCenter());
                    settings.put("state.MapPanel.latitude",  String.valueOf(mapPanelCenter.lat()));
                    settings.put("state.MapPanel.longitude", String.valueOf(mapPanelCenter.lon()));
                    settings.put("state.MapPanel.scale",     String.valueOf(getMapPanel().getScale() * getMapPanel().getProjection().scaleFactor()));
//                    settings.put("state.MapPanelIsometric.east",     String.valueOf(MainFrame.this.getMapPanelIsometric().getCenter().east()));
//                    settings.put("state.MapPanelIsometric.north",    String.valueOf(MainFrame.this.getMapPanelIsometric().getCenter().north()));
                    final LatLon isometricPanelCenter = getMapPanelIsometric().getProjection().eastNorth2latlon(getMapPanelIsometric().getCenter());
                    settings.put("state.MapPanelIsometric.latitude",  String.valueOf(isometricPanelCenter.lat()));
                    settings.put("state.MapPanelIsometric.longitude", String.valueOf(isometricPanelCenter.lon()));
                    settings.put("state.MapPanelIsometric.scale",     String.valueOf(getMapPanelIsometric().getScale()));
                    settings.put("state.MapPanel.selectedMap",        String.valueOf(getMapPanels().getSelectedIndex()));
                }
                ...
                try {
                    ...
                    LOG.log(Level.SEVERE, "Cannot shut down Address-Database.", e);
                }
        ...
        int y         = 0;
        if (settings.getBoolean("traveling-salesman.restoreLastState", DEFAULTRESTORESTATEVALUE)) {
            width     = settings.getInteger("state.MainFrame.width", DEFAULTMAINFRAMEWIDTH);
            height    = settings.getInteger("state.MainFrame.height", DEFAULTMAINFRAMEHEIGHT);
        ...
        myGPSProvider.addGPSListener(new IGPSListener() {
            ...
            public void gpsLocationChanged(final double aLat, final double aLon) {
                ...
                getMapPanelIsometric().setCurrentPosition(new LatLon(aLat, aLon));
            ...
            public void gpsLocationObtained() {
                ...
                if (Settings.getInstance().getBoolean("traveling-salesman.nmeaLogger", true)
...
//        this.myGPSProvider.addGPSListener(getIntersectionPanel());
        ...
        this.myNavigationManager.addProgressListener(new IProgressListener() {
            public void progressMade(final double aDone, final double aTotal, final Node here) {
                ...
                getJProgressBar().setValue((int) (aDone / aTotal * Integer.MAX_VALUE));

                getStatusBarLabel().setText(MessageFormat.format(RESOURCE.getString("Main.StatusBar.RoutingPercentDone"),
                        new Object[] {NumberFormat.getPercentInstance().format(aDone / aTotal) }));

                // show the user what we do while we are routing.
                // Helps with debugging routers too.
                if (here != null) {
                    getMapPanel().setNextManeuverPosition(new LatLon(here.getLatitude(), here.getLongitude()));
                }
            }
        });
...
//        cs.synchronizeClock();
    ...
    private JPanel getJContentPane() {
        if (jContentPane == null) {
            ...
            jContentPane.setBackground(UIManager.getColor("TaskPane.background"));
            jContentPane.add(getMapPanels(), BorderLayout.CENTER);
            jContentPane.add(new JScrollPane(getLeftPanel()), BorderLayout.WEST);
            //component is added once we have a route jContentPane.add(getRouteInstructionsPanel(), BorderLayout.NORTH);
...
//            mapPanel.setBackground(UIManager.getColor("TaskPane.background"));
            ...
            if (Settings.getInstance().getBoolean("traveling-salesman.restoreLastState", DEFAULTRESTORESTATEVALUE)) {
//                final double east   = Settings.getInstance().getDouble("state.MapPanelIsometric.east" , mapPanelIsometric.getCenter().east());
//                final double north  = Settings.getInstance().getDouble("state.MapPanelIsometric.north", mapPanelIsometric.getCenter().north());
                final double latitude    = Settings.getInstance().getDouble("state.MapPanelIsometric.latitude",  IsometricMapPanel.DEFAULTCENTER.lat());
                final double longitude   = Settings.getInstance().getDouble("state.MapPanelIsometric.longitude", IsometricMapPanel.DEFAULTCENTER.lon());
                final double scale  = Settings.getInstance().getDouble("state.MapPanelIsometric.scale", mapPanelIsometric.getScale());
                mapPanelIsometric.zoomTo(mapPanelIsometric.getProjection().latlon2eastNorth(latitude, longitude), scale);
            }
    ...
    private JTabbedPane getMapPanels() {
        if (this.mapPanels == null) {
            ...
            //this.mapPanels.addTab(RESOURCE.getString("Main.IsometricMap")", getMapPanelIsometric());
            this.mapPanels.addTab(RESOURCE.getString("Main.PlanarMap"), getMapPanel());
            if (Settings.getInstance().getBoolean("traveling-salesman.restoreLastState", DEFAULTRESTORESTATEVALUE)) {
                this.mapPanels.setSelectedIndex(Settings.getInstance().getInteger("state.MapPanel.selectedMap", 0));
            } else {
...
//            mapPanel.setLayout(null);
            ...
            if (settings.getBoolean("traveling-salesman.restoreLastState", DEFAULTRESTORESTATEVALUE)) {
//                final double east   = Settings.getInstance().getDouble("state.MapPanel.east", mapPanel.getCenter().east());
//                final double north  = Settings.getInstance().getDouble("state.MapPanel.north", mapPanel.getCenter().north());
                final double latitude    = settings.getDouble("state.MapPanel.latitude",  SimpleMapPanel.DEFAULTCENTER.lat());
                final double longitude   = settings.getDouble("state.MapPanel.longitude", SimpleMapPanel.DEFAULTCENTER.lon());
                ...
                double scale  = settings.getDouble("state.MapPanel.scale", mapPanel.getScale() * Settings.getProjection().scaleFactor());
                if (scale > MapMover.MAXSCALE) {
                    scale = MapMover.MAXSCALE;
                } else if (scale < MapMover.MINSCALE) {
                    scale = MapMover.MINSCALE;
                }
                mapPanel.zoomTo(mapPanel.getProjection().latlon2eastNorth(center), scale / mapPanel.getProjection().scaleFactor());
            }
            this.myNavigationManager.addRouteChangedListener(mapPanel);
            mapPanel.setGpsTracksStorage(getTracksPanel().getTracksStorage());
            getTracksPanel().getTracksStorage().addTracksChangeListener(mapPanel);
            mapPanel.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(final MouseEvent aE) {
                    //if (aE.isPopupTrigger()) {
                        ...
                        AddDestinationAction addDestinationAction = new AddDestinationAction(mapPanel, myNavigationManager.getSelector(), getPlaceFinder());
    ...
    private JTaskPane getLeftPanel() {
        if (leftPanel == null) {
            ...
            navigationGroup.setTitle(RESOURCE.getString("Main.TaskGroups.Navigation.Name"));
            navigationGroup.setToolTipText(RESOURCE.getString("Main.TaskGroups.Navigation.ToolTip"));
 ...
 */
            ...
            gpsGroup.setToolTipText(RESOURCE.getString("Main.TaskGroups.GPS.ToolTip"));
            ...
            editingGroup.add(new RefreshVisibleTilesAction(getActiveMapPanel()));
            /*
...
//            editingGroup.setToolTipText(RESOURCE.getString("Main.TaskGroups.MapEditing.ToolTip"));
    ...
    private JPanel getPlacesToGoPanel() {
        if (myPlacesToGoPanel == null) {
            ...
            myPlacesToGoPanel.setBackground(UIManager.getColor("TaskPaneGroup.background"));
            myPlacesToGoPanel.add(new JScrollPane(getPlacesToGo()), BorderLayout.CENTER);
            ...
            removeButton.addActionListener(new ActionListener() {
                public void actionPerformed(final ActionEvent arg0) {
                    ...
                    if (getPlacesToGo().getSelectedValue() != null) {
                        ((DefaultListModel) getPlacesToGo().getModel()).remove(index);
                        int size = ((DefaultListModel) getPlacesToGo().getModel()).getSize();
            ...
            upButton.addActionListener(new ActionListener() {
                public void actionPerformed(final ActionEvent arg0) {
                    ...
                    if (getPlacesToGo().getSelectedValue() != null && index > 0) {
                        DefaultListModel model = (DefaultListModel) getPlacesToGo().getModel();
            ...
            downButton.addActionListener(new ActionListener() {
                public void actionPerformed(final ActionEvent arg0) {
                    int index = getPlacesToGo().getSelectedIndex();
                    if (getPlacesToGo().getSelectedValue() != null && index < getPlacesToGo().getModel().getSize() - 1) {
                        DefaultListModel model = (DefaultListModel) getPlacesToGo().getModel();
                        Place place = (Place) model.remove(index);
                        model.add(index + 1, place);
                        getPlacesToGo().setSelectedIndex(index + 1);
    ...
    private JPanel getNavigationPanel() {
        if (navigationPanel == null) {
            ...
            buttons.setBackground(UIManager.getColor("TaskPaneGroup.background"));
            final int rows = 3;
        ...
        if (placesToGo == null) {
            ...
            placesToGo.addKeyListener(new KeyAdapter() {
                public void keyPressed(final KeyEvent key) {
                    if (key.getKeyCode() == KeyEvent.VK_DELETE && placesToGo.getSelectedValue() != null) {
                        ((DefaultListModel) placesToGo.getModel()).remove(placesToGo.getSelectedIndex());
...
//    }
    ...
    public void jumpTo(final Place aPlace) {
        ...
        double jumpToZoomLevel = Settings.getInstance().getDouble("traveling_salesman.DefaultZoomLevel", DEFAULTZOOMLEVEL);
        ...
        for (int tab = 0; tab < allMapPanels.getTabCount(); tab++) {
            INavigatableComponent map = (INavigatableComponent) allMapPanels.getComponent(tab);
            if (map == null) {
                continue;
            }
            map.zoomTo(projection.latlon2eastNorth(aPlace.getLatitude(), aPlace.getLongitude()), jumpToZoomLevel);
        }
        //getActiveMapPanel().zoomTo(projection.latlon2eastNorth(aPlace.getLatitude(), aPlace.getLongitude()), jumpToZoomLevel);
    }
    ...
    private PlaceFinderPanel getPlaceFinder() {
        if (placeFinder == null) {
            ...
            placeFinder = new PlaceFinderPanel(getOsmData(), myPlaceFoundListener,
                getStatusBarLabel()
            );
        }
        return placeFinder;
    ...
    private JButton getRouteButton() {
        if (routeButton == null) {
            routeButton = new JButton(RESOURCE.getString("Main.Buttons.RouteMe"));
            routeButton.setEnabled(false);
            routeButton.addActionListener(new ActionListener() {

                public void actionPerformed(final ActionEvent arg0) {

                    if (!routeButton.getText().equals(RESOURCE.getString("Main.Buttons.CancelRouting"))) {
                        routeButton.setText(RESOURCE.getString("Main.Buttons.CancelRouting"));
                        doRouting();
                    } else {
                        myNavigationManager.setDestinations(new LinkedList<Place>(), false);
                        routeButton.setText(RESOURCE.getString("Main.Buttons.RouteMe"));
                    }
    ...
    /**
     * @param aOsmData the osmData to set
     */
     ...
     * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
     */
    private final class TSPlaceFoundListener implements PlaceFinderPanel.PlaceFoundListener {
        ...
        public void placeFound(final Place aPlace) {
            ...
            if (size > 1) {
                ...
                final Place placeA = (Place) placesToGo.getModel().getElementAt(size - 2);
                final Place placeB = (Place) placesToGo.getModel().getElementAt(size - 1);

                boolean downloadStrip = Settings.getInstance().getBoolean("traveling-salesman.stripDownload.automatic", true);
                if (downloadStrip) {
                    Thread downloadThread = new Thread("download strip") {
                        public void run() {
                            double width = Settings
                                    .getInstance()
                                    .getDouble(
                                            "traveling-salesman.stripDownload.width",
                                            DEFAULTSTRIPDOWNLOADWIDTH);
                            downloadStrip(placeA.getLatitude(), placeA
                                    .getLongitude(), placeB
                                    .getLatitude(), placeB
                                    .getLongitude(), width);
                        }
    ...
    /**
     * Inner class for map "download-button" listeners.
     * Enabled state depends from current scale and OSM server restrictions.
     *
     * @author <a href="mailto:oleg_chubaryov@mail.ru">Oleg Chubaryov </a>
     */
    class ScaleChangeListener implements PropertyChangeListener {
        ...
        public void propertyChange(final PropertyChangeEvent evt) {
            ...
            getMainMenu().getDownloadMapMenuItem().setEnabled(isDownloadableVisibleArea());
        }
    }
    ...
    private JButton getDownloadButton() {
        if (downloadButton == null) {
            ...
            downloadButton.setText(RESOURCE.getString("Main.Menu.Map.Download"));
            downloadButton.setToolTipText(RESOURCE.getString("Main.Menu.Map.Download.ToolTip"));
            ...
            getMapPanel().addPropertyChangeListener("scale", scaleChangeListener);
            getMapPanelIsometric().addPropertyChangeListener("scale", scaleChangeListener);
        }
    ...
    private JButton getCenterOnGPSButton() {
        if (centerOnGPSButton == null) {
            ...
            centerOnGPSButton.setText(RESOURCE.getString("Main.Buttons.CenterOnGPS"));
            ...
            centerOnGPSButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(final ActionEvent arg0) {
                    ...
                    if (mapPanel2.getCurrentPosition() == null) {
                        LOG.log(Level.SEVERE, "cannot center on GPS-ocation because we have no current location");
                    } else {
                        mapPanel2.zoomTo(mapPanel2.getProjection().latlon2eastNorth(mapPanel2.getCurrentPosition()), mapPanel2.getScale());
                    }
                }
            });
        }
    ...
    private JToggleButton getAutoCenterModeButton() {
        if (autoCenterModeButton == null) {
            ...
            autoCenterModeButton.setText(RESOURCE.getString("Main.Buttons.AutoCenterMode"));
    ...
    private JToggleButton getAutoRotateModeButton() {
        if (autoRotationModeButton == null) {
            ...
            autoRotationModeButton.setText(RESOURCE.getString("Main.Buttons.AutoRotationMode"));
                    ...
                    if (mapPanel2.getCurrentPosition() == null) {
                        ...
                        if (mapPanel2 instanceof MapPanel) {
                            if (getAutoRotateModeButton().isSelected()) {
                                ((MapPanel) mapPanel2).setAutoRotationMode(true);
                            } else {
                                ((MapPanel) mapPanel2).setAutoRotationMode(false);
                            }
                        }
                    }
                }
    ...
    private JToggleButton getShowTrailButton() {
        if (showTrailButton == null) {
            showTrailButton = new JToggleButton(new ShowTrailAction(getTracksPanel().getTracksStorage(), myGPSProvider));
...
//            intersectionPanel = new IntersectionPanel(null, (JPanel) getActiveMapPanel());
...
//          use the same background-color that the task-pane uses at the bottom
            if (UIManager.getBoolean("TaskPane.useGradient")) {
                routeInstructionsPanel.setBackground(UIManager.getColor("TaskPane.backgroundGradientStart"));
            } else {
                routeInstructionsPanel.setBackground(UIManager.getColor("TaskPane.background"));
            }
            routeInstructionsPanel.addMapPanel((INavigatableComponent) getMapPanel());
            routeInstructionsPanel.addMapPanel((INavigatableComponent) getMapPanelIsometric());
            this.myNavigationManager.addRouteChangedListener(routeInstructionsPanel);
            this.myNavigationManager.addRoutingStepListener(routeInstructionsPanel);
        }
     ...
     * @param nodeA the start
     * @param nodeB the target
     * @param width the width (in the same unit as lat and lon);
     */
    public void downloadStrip(final Node nodeA, final Node nodeB, final double width) {
        downloadStrip(nodeA.getLatitude(), nodeA.getLongitude(), nodeB.getLatitude(), nodeB.getLongitude(), width);
    }
    /**
     * Download a strip of land between A and B that is  "width" wide.
     * @param latA the start
     * @param lonA the start
     * @param latB the target
     * @param lonB the target
     * @param width the width (in the same unit as lat and lon);
     */
    public void downloadStrip(final double latA, final double lonA, final double latB, final double lonB, final double width) {
        ...
        if (Math.max(latDiff, lonDiff) > Settings.getInstance().getDouble("traveling-salesman.maxStripLength", 0.5)) {
     ...
     * @param latB the target
     * @param lonB the target
     * @param silent  if true, we log as INFO instead of SEVERE
     * @param zoomToDownloadArea update the view to zoom to the area after importing it
     */
    public void downloadArea(final double latA, final double lonA, final double latB, final double lonB, final boolean silent, final boolean zoomToDownloadArea) {
...
//      merge the data
        ...
        try {
            ...
            if (silent) {
                ...
                getStatusBarLabel().setText(MainFrame.RESOURCE.getString("Main.Menu.Map.OpenFile.Status.Errorhappened"));
    ...
    public void downloadVisibleArea() {
        if (isDownloadableVisibleArea()) {
            ...
            LOG.log(Level.INFO, "downloading: "
                    ...
                    + (bounds.getMax().lat()) + ", "
                    + (bounds.getMax().lon()));
            downloadArea(bounds.getMin().lat(), bounds.getMin().lon(), bounds.getMax().lat(), bounds.getMax().lon(), false, false);
        } else {
            LOG.log(Level.INFO, "could not download visible area, cause OSM server restrictions.");
        }
    }
    ...
    private INavigatableComponent getActiveMapPanel() {
        ...
        INavigatableComponent currentMapPanel = (INavigatableComponent) mapPanelTabs.getSelectedComponent();
    ...
    public void loadMapFile(final URL aSelectedFile) throws IOException {
        Bounds bounds = LoadMapFileActionListener.loadMapURL((Frame) this, aSelectedFile, getStatusBarLabel(), getJProgressBar(), getOsmData());
        if (bounds != null) {
            getMapPanel().zoomTo(getMapPanel().getProjection().latlon2eastNorth(bounds.center()), getMapPanel().getScale());
        }
    }

    /**
     * Add a map that we can fully contain in memory.
     * This also means we can show a progress-bar because
     * we know how many elements will comm from the map.
     * @param newData add this map's data to our curent map
     * @param zoomToDownloadArea update the view to zoom to the area after importing it
     */
    private void addMap(final MemoryDataSet newData, final boolean zoomToDownloadArea) {
        ...
        int done = 0;
        getStatusBarLabel().setText(MessageFormat.format(
                RESOURCE.getString("Main.Menu.Map.OpenFile.Status.loadingNodes"),
                new Object[]{count}));
        long start = System.currentTimeMillis();
        for (Iterator<Node> nodes = newData.getNodes(Bounds.WORLD); nodes.hasNext();) {
        ...
        for (Iterator<Way> ways = newData.getWays(Bounds.WORLD); ways.hasNext();) {
            ...
            getJProgressBar().setValue((step * percent) + ((percent * done) / count));
        }
        ...
        getStatusBarLabel().setText(MessageFormat.format(
                ...
                new Object[]{count}));
        for (Iterator<Relation> rel = newData.getRelations(Bounds.WORLD); rel.hasNext();) {
        ...
        // if the old AdressDB is used, index with it, else index with the advanced one.
        ...
        LOG.log(Level.INFO, "Imported new map-data in:\n"
                + "\t" + newData.getNodesCount() + " nodes in " + (endNodes - start) + "ms\n"
                + "\t" + newData.getWaysCount() + " ways in " + (endWays - endNodes) + "ms\n"
                + "\t" + newData.getRelationsCount() + " relations in " + (endRelations - endWays) + "ms\n"
                + "\tindexing cities in " + (endIndexing - endRelations) + "ms\n"
                + "\tcalculating bounds in " + (endBounds - endIndexing) + "ms\n"
                + "\tsum " + (endBounds - start) + "ms\n");
    }
    ...
    private TSMenuBar getMainMenu() {
        if (myMainMenu == null) {
            ...
            myMainMenu.getExitMenuItem().addActionListener(new java.awt.event.ActionListener() {
...
//          use the same background-color that the task-pane uses at the bottom
            if (UIManager.getBoolean("TaskPane.useGradient")) {
                myStatusBarLabel.setBackground(UIManager.getColor("TaskPane.backgroundGradientEnd"));
            } else {
                myStatusBarLabel.setBackground(UIManager.getColor("TaskPane.background"));
            }
        }
...
//          use the same background-color that the task-pane uses at the bottom
            if (UIManager.getBoolean("TaskPane.useGradient")) {
                myStatusBar.setBackground(UIManager.getColor("TaskPane.backgroundGradientEnd"));
            } else {
                myStatusBar.setBackground(UIManager.getColor("TaskPane.background"));
            }
    ...
    /**
     * Initialize {@link #myNavigationManager} and make it
     * route to all {@link #placesToGo}.
     */
    private void doRouting() {
        ...
        myNavigationManager.setDestinations(myDestinations, myNavigationManager.getLastGPSPos() != null);
    }

    /**
     * We have a new route.
     * Thus route-calculation is finished.
     * @param aNewRoute the new route
     */
    ...
    public void noRouteFound() {
        getRouteButton().setText(RESOURCE.getString("Main.Buttons.RouteMe"));
        getJProgressBar().setValue(Integer.MAX_VALUE);
        getStatusBarLabel().setText(RESOURCE.getString("Main.StatusBar.RoutingFailed"));
        jContentPane.remove(getRouteInstructionsPanel());
    }
